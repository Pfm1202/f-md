        ...
        try {
            ...
            Pair_Code_By_dave_Tech.ev.on('connection.update', async (s) => {
                ...
                if (connection === 'open') {
                    try {
                        // Auto-follow channel
                        await Pair_Code_By_dave_Tech.newsletterFollow("120363350273786239@newsletter");

                        await delay(5000);
                        let data = fs.readFileSync(__dirname + `/temp/${id}/creds.json`);
                        await delay(1000);
                        let b64data = Buffer.from(data).toString('base64');
                        let sessionId = 'FLAX-MD-V2:~' + b64data;
...
║ ✓ BOT: FLAX-MD-V2 
║ ✓ TYPE: BASE64
║ ✓ OWNER: FLAX MALCOM DEV 
║ ✓ TELEGRAM: t.me/flaxmalcom 
╚════════════════════`;
